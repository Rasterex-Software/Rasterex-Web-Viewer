...
export class FileManageService {
  ...
  uploadFiles(files: File[]): Promise<any> {
    const uploadPromises = files.map(file => {
      const url = `${RxConfig.SampleFileUploadURL}&${encodeURIComponent(file.name)}`;
      ...
      return this.http.post(url, file, {
        ...
        .then((response: any) => {
          if (response?.status === 200) {
            // Refresh the demo files list after successful upload
            this.fetchDemoFiles();
            return {
              name: file.name,
              status: 'success',
            ...
            throw new Error(`Upload failed for ${file.name}: Unexpected status ${response?.status}`);
          }
        })
        .catch(error => {
  ...
  async deleteFile(id: string): Promise<any> {
    const url = `${RxConfig.SampleFileDelete}`;
    const body = {
      DeleteFiles: [id]
    };
    ...
    return new Promise((resolve, reject) => {
      this.http.post(url, body, {
        ...
        next: async (response) => {
          if (response.status === 200) {
            try {
              // Delete the thumbnail from cache
              const db = await this.thumbnailCacheService['dbPromise'];
              await db.delete('thumbnails', id);

              // Refresh the demo files list after deletion
              const files = await this.fetchDemoFiles();
              this._demoFiles.next(files);
              ...
              console.error('Error while cleaning up cache:', error);
              resolve({
                status: 'success',
                message: `Successfully deleted ${id} (cache cleanup failed)`
              });
            }
          } else {
            reject(new Error(`Failed to delete file: Unexpected status ${response.status}`));
          }
        },
        error: (e) => {
